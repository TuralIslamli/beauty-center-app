...
const ReportsTable: React.FC<ReportsTableProps> = ({ userPermissions }) => {
  ...
  }, [dates, debouncedClientName, debouncedClientPhone, doctor?.id, hasPermission]);
  ...
  // Body Templates
  const idBodyTemplate = useCallback((rowData: IReport, options: { rowIndex: number }) => (
    isLoading ? <Skeleton width="20px" /> : <div>{options.rowIndex + 1}</div>
  ), [isLoading]);

  const dateBodyTemplate = useCallback((rowData: IReport) => (
    isLoading ? <Skeleton width="100px" /> : rowData.date_time.slice(0, -3)
  ), [isLoading]);

  const clientNameBodyTemplate = useCallback((rowData: IReport) => (
    isLoading ? <Skeleton width="100px" /> : <div>{rowData.client_name}</div>
  ), [isLoading]);

  const clientPhoneBodyTemplate = useCallback((rowData: IReport) => (
    isLoading ? <Skeleton width="100px" /> : <div>{formatPhone(rowData.client_phone)}</div>
  ), [isLoading]);

  const doctorBodyTemplate = useCallback((rowData: IReport) => (
    isLoading ? <Skeleton width="150px" /> : `${rowData.user?.name} ${rowData.user?.surname}`
  ), [isLoading]);

  const serviceTypeBodyTemplate = useCallback((rowData: IReport) => (
    isLoading ? (
  ...
  const priceBodyTemplate = useCallback((rowData: IReport) => (
  ...
  const allowExpansion = useCallback((rowData: IReport) => (
    (rowData.amount_change_history?.length ?? 0) > 1
  ), []);

  const dateRowTemplate = useCallback((rowData: IAmountChangeHistory) => (
    isLoading ? <Skeleton width="100px" /> : rowData.created_at.slice(0, -3)
  ), [isLoading]);

  const causerBodyTemplate = useCallback((rowData: IAmountChangeHistory) => (
    isLoading ? (
  ...
  const rowExpansionTemplate = useCallback((data: IReport) => (
    <div className="p-3">
      <DataTable value={data.amount_change_history}>
        ...
        <Column field="status" header="Qəbul edən" body={causerBodyTemplate} />
      </DataTable>
    </div>
    ...
    <>
      ...
      {hasPermission('service.get_all.total_amount') && (
        <div className="total-info">
          <Message className="info-message" severity="info" content={totalContent} />
        </div>
      )}
